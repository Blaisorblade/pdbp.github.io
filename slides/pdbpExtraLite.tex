...
\item<9-> \ttdg{implicit object}'s \emdb{define natural transformations}
\item<10-> \emdb{program meanings} use them 
\begin{itemize}
\item<11-> to \emdb{transform program implementations}
\end{itemize}
\end{itemize}
\end{itemize}
\end{frame}
...
\frametitle{\begin{center}\ttdg{PDBP} library design decisions\end{center}}
\begin{itemize}
\item<2-> programs are \emdb{defined} in \ttdg{class}'es that are \emdb{declared} to \emdb{implicitly} have programming capabilities available
\end{itemize}
\begin{itemize}
\item<3-> programs are \emdb{implemented} using \emdb{dependency injection} by \ttdg{import}
\end{itemize}
\end{frame}
...
object FactorialOfIntReadWritingToConsoleWrittenToConsoleMain
  ...
  def main(args: Array[String]): Unit = {
    ...
    import ... implicits.activeIntReadingMeaningOfActiveIntReadingWithWritingToConsole.meaning
