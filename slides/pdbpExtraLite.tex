...
\item<3-> Can you think of a more meaningful name?
\end{itemize}
\end{center}
\end{frame}
...
\begin{center}
\begin{itemize}
\item<2-> \ttdg{factorial} \emdb{definition} uses the capabilities \emdb{declared} in \\ \emdb{type class} \ttdg{trait Program[>-->[- \_, + \_]]}
\end{itemize}
\end{center}
\end{frame}
...
\begin{center}
\begin{itemize}
\item<2-> \ttdg{factorial} \emdb{implementations} depend on \ttdg{implicit object}'s \emdb{defining} the capabilities of type class \\ \ttdg{trait Program[>-->[- \_, + \_]]}
\end{itemize}
\end{center}
\end{frame}
...
\begin{itemize}
\item<5-> \ttdg{FP} \emdb{semantics} is \emdb{fixed} \\ 
\ttdg{PDBP} \emdb{semantics} is \emdb{not fixed}
\item<6-> \ttdg{FP} \emdb{capabilities} are \emdb{fixed} \\ 
\ttdg{PDBP} \emdb{capabilities} are \emdb{not fixed}
\item<7-> \ttdg{FP} \emdb{effects} are \emdb{impure} \\ 
...
\end{itemize}
\begin{itemize}
\item<3-> \emdb{binding} based, \emdb{pointful} functional programming
\end{itemize}
\begin{itemize}
\item<4-> \ttdg{\{ val z = ez ; \{ val y = ey ; ex(z,y) \} \} }
\item<5-> \ttdg{mz bind \{ z => my bind \{ y => mx(z,y) \} \} }
\item<6-> \ttdg{mz bind \{ z => my bind \{ y => result(ex(z,y)) \} \} }
\end{itemize}
\end{frame}
...
\begin{itemize}
\item<3-> \emdb{monads} allow more \emdb{description liberty}
...
\begin{frame}[fragile]
\frametitle{\begin{center}Uh! Oh! typo \ldots or error?\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
  val factorial: BigInt `=>C` BigInt = { z =>
...
\begin{itemize}
\item<2-> \emdb{description} separated from \emdb{implementations} and \emdb{meanings}
\end{itemize}
\begin{itemize}
\item<3-> \emdb{description}
\begin{itemize}
\item<4-> \ttdg{trait}'s (\emdb{type classes}) that \emdb{declare} capabilities 
\end{itemize}
\item<5-> \emdb{implementations}
\begin{itemize}
\item<6-> \ttdg{implicit object}'s that \ttdg{extend} \ttdg{trait}'s \emdb{define} capabilities 
\end{itemize}
\item<7-> \emdb{meanings}
\begin{itemize}
\item<8-> \ttdg{implicit object}'s that \emdb{define} a \emdb{natural transformation}
\end{itemize}
\end{itemize}
\end{frame}
...
\item<3-> Solution: \emdb{transformation} \ttdg{FreeTransformation} and \emdb{meaning} \ttdg{FreeTransformedMeaning}
\end{itemize}
\begin{itemize}
\item<4-> Problem: \ttdg{effectfulReadIntFromConsole} and \ttdg{effectfulWriteFactorialOfIntToConsole} \emdb{execute effects}
\item<5-> Solution: \ttdg{Reading} resp. \ttdg{Writing} extensions of \ttdg{Program}
with members \ttdg{read} resp. \ttdg{write} that \emdb{describe effects} and with corresponding \emdb{transformation} and \emdb{meaning}.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item<2-> \emdb{Computation transformations} transform \emdb{computations} (and corresponding \emdb{kleisli programs}) to \emdb{computations} (and corresponding \emdb{kleisli programs}) 
\end{itemize}
\end{frame}
...
\begin{itemize}
\item<2-> greatly reduces \emdb{reading} boilerplate code
\end{itemize}
\end{frame}
...
object FactorialOfIntReadWritingToConsoleWrittenToConsoleMain {
  ...
  import ... readIntFromConsoleEffect // actual read effect
  import ... writeFactorialOfIntReadFromConsoleToConsoleEffect // actual write effect
