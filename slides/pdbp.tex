...
\begin{frame}[fragile]
\frametitle{\begin{center}\ttdg{Console} \emdb{I/O Effect descriptions}\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
  type Effect = Unit => Unit
...
\begin{frame}[fragile]
\frametitle{\begin{center}\ttdg{factorialOfIntReadWithConsoleEffectMain} part 1\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
  val readInt: 
  ...
  def `z>-->console`[Z]
    : implicit WriteToConsoleEffect[Z] => Z >--> Console = 
    function { z =>
      val implicitWriteToConsoleEffect = implicitly
      Console(implicitWriteToConsoleEffect(z))  
    }

  def writeToConsole[Z]
    : implicit WriteToConsoleEffect[Z] => Z >--> Unit =
    `z>-->console` >--> `w>-->u`
\end{verbatim}
}
}
...
\begin{frame}[fragile]
\frametitle{\begin{center}\ttdg{factorialOfIntReadWithConsoleEffectMain} part 2\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
  def factorialOfIntReadWithConsoleEffectMain
    (implicit effect: WriteToConsoleEffect[BigInt])
    : Unit >--> Unit =
    readInt >-->
      factorial >-->
      writeToConsole 
...
\begin{frame}[fragile]
\frametitle{\begin{center}\ttdg{programMeaning} part 2\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
  trait ProgramMeaning[
...
\begin{frame}[fragile]
\frametitle{\begin{center}Running part 1\end{center}}
\begin{itemize}
\item<2-> type classes
\item<3-> dependency injection by \ttdg{import}
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{\begin{center}Running part 2\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
  val executeWrittenToConsoleEffect: Console && Unit => Unit = {
    case (Console(effect), _) =>
      effect(())
  }
\end{verbatim}
}
...
\begin{frame}[fragile]
\frametitle{\begin{center}Running part 3\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
  implicit val implicitReadIntFromConsoleEffect
    : ReadFromConsoleEffect[BigInt] =
    readIntFromConsoleEffect(
      "please type an integer to read") 

  implicit val implicitWriteIntToConsoleEffect
    : WriteToConsoleEffect[BigInt] =
    writeToConsoleEffect(
      "the factorial value of the integer read is")
      

  val effectfulMain: Unit = 
    executeWrittenToConsoleEffect(
      programMeaning(
        factorialOfIntReadWithConsoleEffectMain)(()))
\end{verbatim}
}
}
\end{frame}
