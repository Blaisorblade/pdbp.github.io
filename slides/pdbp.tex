...
\begin{itemize}
\item<2-> This is a painting by Ren\'{e} Magritte
\end{itemize}
\end{frame}
...
\item<3-> different \ttdg{implicit object}'s \emdb{define} capabilities of \\ type class \ttdg{trait Program[>-->[- \_, + \_]]}
\end{itemize}
\end{center}
\end{frame}
...
\item<3-> \ttdg{trait Computation[+ \_]} corresponds to \emdb{monads}
\end{itemize}
\end{frame}
...
\item<4-> \ttdg{\{ val z = ez ; \{ val y = ey ; /* ... */ \} \} }
\item<5-> \ttdg{mz bind \{ z => my bind \{ y => /* ... */ \} \} }
\end{itemize}
\item<6-> \emdb{arrows} are \emdb{programs} that generalize \emdb{functions}
\begin{itemize}
\item<7-> \emdb{composition} based, \emdb{pointfree} functional programming
\item<8-> \ttdg{val `z=>x` = `z=>y` andThen `y=>x`}
\item<9-> \ttdg{val `z>-->x` = `z>-->y` >--> `y>-->z`}
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item<6-> \ttdg{implicit object}'s \emdb{define} capabilities (\ttdg{extend} \ttdg{trait}'s)
\end{itemize}
...
\begin{frame}[fragile]
\frametitle{\begin{center}\ttdg{activeReadingWithWritingTypes}\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
