...
\begin{frame}[fragile]
\frametitle{\begin{center}\ttdg{IdentityMeaning}\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
...
\begin{verbatim}
  private[pdbp] trait FreeTransformedMeaning[
    ...
    // Result(z) => result(z)
    // Bind(Result(y), y2ftcz) => fold(y2ftcz(y))
    // Bind(Bind(cx, x2ftcy), y2ftcz) => 
    //   fold(Bind(cx, Bind(x2ftcy, y2ftcz)))
...
\begin{verbatim}
  private[pdbp] trait ReadingTransformation[
    R, C[+ _]: Computation]
      ...
      with Reading[
        R, 
        Kleisli[ReadingTransformed[R, C]]] {
...
\begin{verbatim}
  private[pdbp] trait WritingTransformation[
    W: Writable, C[+ _]: Computation]
      extends ComputationTransformation[
        C, WritingTransformed[W, C]]

      with Writing[
        W, 
        Kleisli[WritingTransformed[W, C]]] {
