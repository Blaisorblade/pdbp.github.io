...
\begin{center}
\begin{itemize}
\item<2-> \ttdg{factorial} \emdb{description} uses the capabilities \emdb{declared} in \\ type class \ttdg{trait Program[>-->[- \_, + \_]]}
\end{itemize}
\end{center}
\end{frame}

\begin{frame}[fragile]
\frametitle{\begin{center}\ttdg{factorial} definitions\end{center}}
{\color{DarkGreen}	
\footnotesize{
\begin{verbatim}
  val factorial: BigInt >--> BigInt =
...
\begin{center}
\begin{itemize}
\item<2-> \ttdg{factorial} \emdb{definitions} use \ttdg{implicit object}'s \emdb{defining} the capabilities of \\ type class \ttdg{trait Program[>-->[- \_, + \_]]}
\end{itemize}
\end{center}
\end{frame}
...
\begin{center}
\begin{itemize}
\item<2-> \ttdg{factorial} \emdb{meanings} use \emdb{natural transformations}
\end{itemize}
\end{center}
\end{frame}
...
\frametitle{\begin{center}Arrows versus monads\end{center}}
\begin{itemize}
\item<2-> \emdb{monads} (\ttdg{trait Computation[C[+ \_]]}) \\ generalize \emdb{expressions} 
\begin{itemize}
...
\begin{itemize}
\item<2-> \emdb{description} separated from \emdb{definition} and \emdb{meaning}
\end{itemize}
\begin{itemize}
...
\end{itemize}
\item<5-> \emdb{definition}
\begin{itemize}
\item<6-> \ttdg{implicit object}'s (\ttdg{extend} \ttdg{trait}'s) \emdb{define} capabilities 
\end{itemize}
\item<7-> \emdb{meaning}
\begin{itemize}
\item<8-> \ttdg{implicit object}'s having \emdb{natural transformation} member
\end{itemize}
\end{itemize}
\end{frame}
...
\footnotesize{
\begin{verbatim}
// same mainFactorial description
import ... mainFactorial
import mainFactorial.factorialMain
...
object FactorialOfIntReadMain {
  ...
  import ... readIntFromConsoleEffect // executes read effect
...
object FactorialOfIntReadWritingToConsoleWrittenToConsoleMain {
  ...
  import ... readIntFromConsoleEffect // executes read effect
  import ... writeFactorialOfIntReadFromConsoleToConsoleEffect // executes write effect
  import ... writeToConsoleEffect // executes write effect via writeUsing
